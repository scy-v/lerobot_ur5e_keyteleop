...
> [!NOTE]
> 本仓库基于 UR RTDE 和 LeRobot，支持笛卡尔位置/阻抗控制、键盘遥操作与数据采集。
...
| 仓库 | 功能 | 链接 |
|:----:|:-----|:----:|
| **lerobot_ur5e_isoteleop** | 基于 LeRobot 构建的 UR5e/UR7e 主从臂遥操作框架，具有关节级同构映射。 | [GitHub](https://github.com/scy-v/lerobot_ur5e_isoteleop) |
| **lerobot_ur_dual_vrteleop** | 基于 LeRobot 和 PICO 4 Ultra 构建的 UR5e/UR7e 双臂 VR 遥操作框架。 | [GitHub](https://github.com/scy-v/lerobot_ur_dual_vrteleop) |
...
- `record.cameras.enabled: false`：不打开 RealSense 相机，改用固定假画面。
- `record.robot.use_gripper: false`：不初始化夹爪硬件，也不包含夹爪 action；夹爪 observation 值保持为 `0`。

### 2.1 获取 RealSense 相机序列号（可选）

设置 `record.cameras.enabled: false` 后，将使用固定假图像进行录制，不会导入或打开 RealSense 硬件。启用相机时，在分别获取序列号的过程中请确保每次仅连接一个相机：
...
- `cameras.enabled`：为 `True` 时使用配置的 RealSense 相机；为 `False` 时跳过 RealSense 导入和硬件检查，并为 `wrist_image` 与 `exterior_image` 输出固定假画面。
- `cameras.width` 和 `cameras.height`：真实相机与假相机共用的分辨率。
- `cameras.dummy_value`：假 RGB 图像的像素值，范围为 `[0, 255]`。
...
- `reference_frame`：`"base"` 或 `"tcp"`。
  ...
  - 为避免 UR 示教器 TCP offset 变化导致坐标定义不一致，`tcp_pose` 字段实际存储由 UR TCP pose 和 TCP offset 转换得到的 EE/flange 位姿。
  - 采集数据中的 `tcp_pose` 和 action 使用 EE/flange 语义；除此之外，其他的 UR 状态读取、复位位姿和底层控制命令仍使用 UR TCP 语义。
- `init_pose`：复位 TCP 位姿 `[x, y, z, roll, pitch, yaw]`；旋转在配置中使用 XYZ 欧拉角，单位为弧度，内部会转换为 UR 需要的旋转向量。
- `init_pose_range`：复位位姿 `[x, y, z, roll_deg, pitch_deg, yaw_deg]` 的随机范围；旋转范围单位为度。
- `teleop.position_step_size`：position 控制下的键盘平移增量。
- `teleop.force_step_size`：force 控制下的键盘平移增量。
- `teleop.force_fine_step_size`：按 `/` 切换后的 force 控制键盘平移增量。
- `teleop.position_rot_step_size`：position 控制下的键盘欧拉角增量。
- `teleop.force_rot_step_size`：force 控制下的键盘欧拉角增量。
- `teleop.force_fine_rot_step_size`：按 `/` 切换后的 force 控制键盘欧拉角增量。
- `robot.force_mode.hold_current_pose_on_idle`：为 `True` 时，force 控制在没有运动按键输入时保持当前实际 TCP 位姿。
...
1. **右方向键**
   ...
   2. 按回车键进入**复位**阶段。此时可继续用键盘遥操作，先将机器人退到安全的预复位位置。
   3. 按下右方向键结束人工复位控制，随后机器人会自动复位到 `cfg.yaml` 中配置的初始 TCP 位姿。
   4. 再次按回车键，开始录制下一个 episode。
...
4. **Ctrl+C 或抛出异常**
   - 按下或发生异常时：自动进入异常处理阶段，提示用户是否将当前未完成的数据集移动到回收站。
...
1. 数据集默认保存在 `~/.cache/huggingface/lerobot` 目录下，包含三类内容：
   ...
   - `dataset_info.txt`：自动记录本地数据集信息，包括以下字段：`record_id`、`name`、`task`、`date`、`version`、`user_info` 和 `type`。其中，`user_info` 可以通过 `cfg.yaml` 中的 `user_notes` 进行注解；录制结束后，`user_info` 会自动追加本次运行的 `record_time`、`reset_time`、`total_time`、`avg_record_time` 和 `avg_reset_time`。
